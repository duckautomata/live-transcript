...
The bell icon on the sidebar opens the Notifications page: create a free account and set up **notification events** that post to your own Discord webhooks when a streamer goes live, schedules a stream, or publishes a video or short, with your own message, pings and embed. Events belong to your account and are visible to nobody else. The page also shows whether the server's live detection is healthy and what it recently detected. An account can be shared by a whole mod team: everyone signs in at once, and **Sessions** in the account menu shows who is signed in and can sign any of them out. The sign-up and change-password forms can generate a 32-character random password for you (it is copied to the clipboard - keep it in a password manager). Accounts are bearer-token sessions kept in local storage; see the server's README for the security model.

Each event is edited on its own page (`/notifications/events/new/` or `/notifications/events/<id>/`) as four steps - when, where, what it says, how often - beside a server-rendered Discord preview. The draft lives in a small store (`src/logic/eventDraft.js`) that every field subscribes to individually, so typing re-renders one input, and it is mirrored to session storage under `lt-event-draft:<account>:<channel>:<id>` so a browser tab switch, a reload or a session expiry never loses it; saving, discarding or signing out clears it. The preview is re-rendered only when the message or card changes, once typing pauses, and the last rendering stays on screen while the next one loads.
